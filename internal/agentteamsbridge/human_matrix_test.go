...
func TestKubernetesHumanMatrixClientLogsInAsBoundHuman(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case request.URL.Path == "/_matrix/client/v3/login":
			...
			if identifier["user"] != "mission-owner" || body["password"] != "test-only-password" {
	...
	human := &unstructured.Unstructured{Object: map[string]any{
		...
		"status":   map[string]any{"phase": "Active", "matrixUserID": "@mission-owner:matrix.test", "initialPassword": "test-only-password"},
	}}
	dynamicClient := dynamicfake.NewSimpleDynamicClient(runtime.NewScheme(), human)
	client, err := agentteamsbridge.NewKubernetesHumanMatrixClient(dynamicClient, "haowork-public", agentteamsbridge.MatrixV3Config{BaseURL: server.URL})
	if err != nil {
		t.Fatal(err)
	}
